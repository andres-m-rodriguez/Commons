...
namespace Commons.Pagination.Blazor;

/// <summary>
/// Reusable state management for cursor-based pagination with Blazor Virtualize.
/// Thread-safe and disposable. Use this when you need pagination state separate from UI.
/// </summary>
/// <typeparam name="TItem">The type of items being paginated.</typeparam>
/// <typeparam name="TParams">The parameters type, must inherit from CursorParameters.</typeparam>
public sealed class CursorPaginationState<TItem, TParams> : IDisposable
    ...
    )
    {
        ...
        while (_hasMoreItems && _cachedItems.Count < endIndex)
        {
            ...
            try
            {
                ...
                if (fetchedCount < baseParams.PageSize)
                {
                    _hasMoreItems = false;
                }
            }
